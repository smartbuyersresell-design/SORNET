...
function renderAccountUI(){
  ...
  if(signedIn){
    ...
    animateElement(avBtn, 'slideInDown');
  } else {
...
function openSignin(){
  ...
  animateElement(signinModal, 'popIn');
}
...
/* ---- Account / settings menu open+close ---- */
...
function toggleAccountMenu(){ 
  if(accountMenu.classList.contains('show')) {
    ...
    animateElement(accountMenu, 'slideInDown');
  }
}
...
document.addEventListener('click', e => {
  if(!accountMenu.contains(e.target) && e.target.id !== 'settingsBtn' && e.target.id !== 'avatarBtn') {
    closeAccountMenu();
  }
});
...
function animateElement(el, animationType) {
  if(!el) return;
  el.style.animation = `${animationType} 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards`;
}

function shakeElement(el) {
  if(!el) return;
  el.style.animation = 'shake 0.4s cubic-bezier(0.36, 0, 0.66, -0.56) forwards';
...
function renderHistory(){
  ...
  history.forEach((item, idx) => {
    ...
    row.addEventListener('click', () => {
      closeHistorySidebar();
      // Add smooth transition before search
...
function openHistorySidebar(){
  ...
  animateElement(historySidebar, 'slideInDown');
}
...
function openDownloadsSidebar(){
  ...
  overlay.classList.add('show');
  animateElement(downloadsSidebar, 'slideInDown');
}
...
function renderTabs(){
  ...
  tabs.forEach((tab, idx) => {
    ...
    tabBtn.addEventListener('click', () => {
      setActiveTab(tab.id);
      animateElement(tabBtn, 'fadeInUp');
    });
  ...
  newTabBtn.addEventListener('click', () => {
    createTab();
    animateElement(newTabBtn, 'rotateIn');
  });
...
function showHome(){
  ...
  resultsView.style.pointerEvents = 'none';
  setTimeout(() => {
...
function showResults(query){
  ...
  resultsView.style.pointerEvents = 'auto';
...
document.getElementById('homeForm').addEventListener('submit', e => {
  ...
  if(q) {
    animateElement(document.getElementById('homeForm'), 'fadeInUp');
    searchQuery(q);
  }
});
...
function renderResults(query, data){
  ...
  if(hasAbstract){
    ...
    card.style.animation = 'fadeInUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
    card.innerHTML = `
  ...
  related.forEach((item, idx) => {
    ...
    row.style.animation = `fadeInUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) ${idx * 50}ms forwards`;
    row.style.opacity = '0';
    row.innerHTML = `
  ...
  footer.style.animation = 'fadeInUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
