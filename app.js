...
function renderHistory(){
  ...
  if(history.length === 0){
    historyContent.innerHTML = '<div style="padding:30px 12px; text-align:center; color:var(--muted); font-size:13px;">No searches yet</div>';
    return;
  }
...
function renderDownloads(){
  ...
  if(downloads.length === 0){
    downloadsContent.innerHTML = '<div style="padding:30px 12px; text-align:center; color:var(--muted); font-size:13px;">No downloads yet</div>';
    return;
  }
...
function showRedirectCard(eng, query){
  ...
  resultsContent.innerHTML = `
    <div class="redirect-card">
      <div class="eng-badge ${eng.cls}">${eng.badge}</div>
      ...
      <a class="btn-solid" href="${eng.urlFn(encodeURIComponent(query))}" target="_blank" rel="noopener">Reopen ${eng.label}</a>
    </div>`;
}
...
function renderResults(query, data){
  ...
  if(hasAbstract){
    ...
    const img = data.Image ? `https://duckduckgo.com${data.Image}` : '';
    card.innerHTML = `
...
function draw(){
  ...
  for(const s of stars){
    ...
    ctx.fillStyle = isDark ? `rgba(230,225,255,${alpha})` : `rgba(107,78,255,${alpha*0.5})`;
    ctx.fill();
  }
